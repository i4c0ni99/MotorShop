...
if (isset($_SESSION['user'])) {
    ...
    if (isset($_POST['delete_offer'])) {
        ...
        $delete_offer = "DELETE FROM offers WHERE subproduct_id = ?";
        $stmt = $mysqli->prepare($delete_offer);
    
        if ($stmt) {
            $stmt->bind_param('i', $subproduct_id);
            $stmt->execute();
        ...
        // Assicurati di passare l'ID del prodotto corretto per il redirect
    ...
    if (isset($_POST['add_offer'])) {
        ...
        $check_offer_query = "SELECT COUNT(*) as count FROM offers WHERE subproduct_id = ?";
        $check_stmt = $mysqli->prepare($check_offer_query);
    
        if ($check_stmt) {
            $check_stmt->bind_param('i', $subproduct_id);
            $check_stmt->execute();
            ...
            if ($count > 0) {
                ...
                header('Location: /MotorShop/subproduct-list.php?id=' . $productId); // Aggiungere una scritta di errore senza redirect
                exit;
            }
