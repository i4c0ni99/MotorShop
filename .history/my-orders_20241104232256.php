...
function cancelOrder($orderId) {
   ...
   $deleteHasQuery = "DELETE FROM orders WHERE id = '{$orderId}'";
   
   // ripristinare le quantity dei sub, con cambio da 0 a 1 di availability eventualmente
   foreach($products as $id => $quantity) {
    $mysqli->query("UPDATE sub_products SET quantity = quantity - " . intval($quantity) . ", availability = 0 WHERE id = " . intval($prod['subproduct_id']));
   }
