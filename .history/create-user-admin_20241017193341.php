...
$usersQuery = "SELECT users.email, users.name, users.surname, groups_id FROM users 
...
if ($usersResult && $usersResult->num_rows > 0) {
    while ($user = $usersResult->fetch_assoc()) {
        // Usa setContent correttamente per ogni variabile separata
        ...
        $body->setContent("toggle_role_link", "/MotorShop/create-user-admin.php?toggleRole=" . urlencode($user['email']));
    }
} else {
    // Se non ci sono utenti da mostrare
