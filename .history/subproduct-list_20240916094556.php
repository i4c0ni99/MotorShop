...
if (isset($_GET['id'])) {
    ...
    if ($sub_result->num_rows > 0) {
        ...
        foreach ($subProducts as $subProduct) {
            ...
            $offer_query = "SELECT percentage FROM offers WHERE subproduct_id = " . (int)$subProduct['id'] . " LIMIT 1";
            $offer_result = $mysqli->query($offer_query);
            if ($offer_result->num_rows > 0) {
                $offer = $data->fetch_assoc();
                $body->setContent("percentage", $offer);
            } else {
                $body->setContent("percentage", "NON ATTIVO");
            }
