...
if (isset($_POST['post-review'])) {
  ...
  // Verifica se l'utente è loggato e se i dati richiesti sono presenti
  if (isset($_SESSION['user']) && !empty($_POST['review']) && !empty($_POST['rate'])) {
      ...
      $rating = intval($_POST["rate"]); // Converte il rating in intero per sicurezza
      $curdate = date("Y/m/d");
      ...
      if ($rating >= 1 && $rating <= 5) {
          ...
          echo "Errore: Valutazione non valida. Il rating deve essere compreso tra 1 e 5.";
      }
