...
if (isset($_SESSION['user']) & if (isset($_GET['id'])) {
    ...
    $oid = $mysqli->query("SELECT id, sub_products_id, quantity FROM orders_has_products WHERE order_id = '{$order_id}'");
    ...
    if ($oid && $oid->num_rows > 0) {
        foreach ($oid as $sub) {
            ...
            if ($sub_product_query && $sub_product_query->num_rows > 0) {
                ...
                $body->setContent("price", formatPrice($sub_product_data['price'])); // Utilizzo di formatPrice()
