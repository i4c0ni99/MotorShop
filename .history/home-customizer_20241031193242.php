...
if (isset($_SESSION['user']) && $_SESSION['user']['groups'] == '1') {
    ...
    if (isset($_POST['edit'])) {
        // Modifica pagina slider
        $title = $mysqli->real_escape_string($_POST['titleEdit']);
        $description = $mysqli->real_escape_string($_POST['descriptionEdit']);

        $insertQuery = "UPDATE slider (title, description) 
        SET ('$title', '$description')"
        WHERE id = $_POST['titleEdit'];
        if ($mysqli->query($insertQuery)) {
            $product_id = $mysqli->insert_id;
        }
    }
