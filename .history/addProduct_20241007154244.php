...
if (isset($_GET['brand_id'])) {
    ...
    if (!empty($brand_id)) {
        $brand = $mysqli->real_escape_string($brand_id);
    } else {
        echo "Brand ID non è valido.";
        $brand = "NULL"; // Imposta un valore di default se necessario
    }
} else {
    $brand = "NULL"; // Imposta un valore di default se non è presente
}
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($errors)) {
        ...
        // if ($result->num_rows > 0) {
            ...
            $details = isset($_POST['details']) ? $mysqli->real_escape_string($_POST['details']) : '';
    ...
    $product_image = isset($_FILES['product_image']) ? $_FILES['product_image'] : null;
            ...
            $insertQuery = "INSERT INTO products (code, title, description, availability, specification, brand_id, categories_id, subcategories_id) 
                            VALUES ('$code', '$title', '$description', 0, '$details', '$brand', ".$_GET['cat_id'].",".$_GET['sub_cat_id'].")";
