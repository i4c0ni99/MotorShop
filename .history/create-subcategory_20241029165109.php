...
if ($subcategories && $subcategories->num_rows > 0) {
    while ($subcategory = $subcategories->fetch_assoc()) {
        // Conta il numero di prodotti per questa sottocategoria
        $productCountResult = $mysqli->query("SELECT COUNT(*) as product_count FROM products WHERE subcategories_id = {$subcategory['id']}");
        $productCount = $productCountResult->fetch_assoc()['product_count'];
        
        // Imposta i valori nel template
        $body->setContent('product_count', $productCount);
        $body->setContent('sub-category', "<a href='create-subcategory.php?categories_id={$category_id}'>{$subcategory['name']}</a>");
        $body->setContent('id', $category_id);
        $body->setContent('delete-url', "/MotorShop/create-category.php?elimina={$subcategory['id']}");
    }
} else {
