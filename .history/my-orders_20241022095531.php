...
WHERE 
    ...
    if ($order_status_query && $order_status_query->num_rows = 1) {

        $shipping_address_data = $shipping_address_query->fetch_assoc();
        $body->setContent("ad_name", $shipping_address_data['name']);
        $body->setContent("ad_surname", $shipping_address_data['name']);
        $body->setContent("ad_province", $shipping_address_data['name']);
        $body->setContent("ad_city", $shipping_address_data['name']);
        $body->setContent("ad_street", $shipping_address_data['name']);
        $body->setContent("ad_cap", $shipping_address_data['name']);
        $body->setContent("ad_phone", $shipping_address_data['name']);
