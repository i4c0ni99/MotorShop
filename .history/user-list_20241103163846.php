...
require "include/template2.inc.php"; /
require "include/dbms.inc.php"; lude il file di gestione del database

$main = new Template("skins/multikart_all_in_one/back-end/frame-private.html"); // Inizializza il template principale
$body = new Template("skins/multikart_all_in_one/back-end/user-list.html"); // Inizializza il template per la lista utenti
...
if (isset($_POST['change_role'])) { // Controlla se è stato inviato un cambiamento di ruolo
    if (isset($_POST['selected_user'])) {
        $email = $mysqli->real_escape_string($_POST['selected_user']); // Sanifica l'input
        $result = $mysqli->query("SELECT groups_id FROM users_has_groups WHERE users_email = '$email'"); // Ottiene il ruolo attuale
        if ($result) {
            $row = $result->fetch_assoc();
            $currentRole = $row['groups_id'];
            $newRole = ($currentRole == 1) ? 2 : 1; // Cambia il ruolo
            $updateQuery = "UPDATE users_has_groups SET groups_id = '$newRole' WHERE users_email = '$email'"; // Query per aggiornare il ruolo
