...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    try {
        $stmt = $mysqli->prepare("
            SELECT users.email, users.password, users.name, users.surname, users.phone, groups_has_services.groups_id 
            FROM users
        ...
        if ($data) {
            // Salva tutte le informazioni dell'utente nella sessione
            $_SESSION['user'] = [
                'email' => $data['email'],
