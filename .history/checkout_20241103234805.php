...
$query = "SELECT c.subproduct_id, c.quantity, sp.products_id, sp.price, 
                 sp.quantity AS prod_quantity, sp.availability, sp.color, sp.size, 
                 i.imgsrc, i.id 
          FROM cart c 
...
// Controlla se ci sono elementi nel carrello
if ($result && $result->num_rows > 0) {
    while ($cartItem = $result->fetch_assoc()) {
        $subproductId = $cartItem['subproduct_id'];
        $sub_quantity = $cartItem['prod_quantity']; // Corrected variable name

        // Aggiungi logica per calcolare il prezzo totale, se necessario
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['address_list']) && isset($_POST['payment_method'])) {
    ...
    $queryCart = "SELECT c.subproduct_id, c.quantity AS cart_quantity, sp.products_id, sp.price, 
                          sp.quantity AS stock_quantity, sp.availability, sp.color, sp.size, 
                          i.imgsrc, i.id 
                   FROM cart c 
                   JOIN sub_products sp ON c.subproduct_id = sp.id 
                   INNER JOIN images i ON sp.products_id = i.product_id 
        ...
        while ($row = $resultCart->fetch_assoc()) {
            ...
            if ($availability == 1 && $cartQuantity <= $stockQuantity) {
                ...
                $products[] = [
                    ...
                    'subtotal' => $subtotal,
                    'quantityCheck' => $stockQuantity - $cartQuantity
                ];
            } else {
