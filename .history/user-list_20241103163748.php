<php session_start(); // Avvia la sessione dell'utente require "include/template2.inc.php" ; // Include il file di
    template principale require "include/dbms.inc.php" ; // Include il file di gestione del database $main=new
    Template("skins/multikart_all_in_one/back-end/frame-private.html"); // Inizializza il template principale $body=new
    Template("skins/multikart_all_in_one/back-end/user-list.html"); // Inizializza il template per la lista utenti if
    (isset($_SESSION['user']) && $_SESSION['user']['groups']=='1' ) { // Verifica se l'utente è loggato e ha i permessi
    $current_user_email=$_SESSION['user']; // Recupera l'email dell'utente attuale // Funzione per caricare gli utenti
    function loadUsers($mysqli, $current_user_email, $searchQuery=null) { global $body; // Rende accessibile il template
    del corpo // Query per selezionare gli utenti $query="SELECT users.name, users.surname, users.email, groups.roul
FROM users
JOIN users_has_groups ON users.email = users_has_groups.users_email
JOIN groups ON groups.id = users_has_groups.groups_id
WHERE users.email != ?" ; // Esclude l'utente attuale if ($searchQuery) { // Se è presente una query di ricerca $query
    .=" AND (users.name LIKE ? OR users.surname LIKE ? OR users.email LIKE ?)" ; $stmt=$mysqli->prepare($query);
    ...
    $body->setContent('src', $row['roul'] == 'Admin' ? '<input id="checkall" class="checkbox_animated check-it"
        ...
        (isset($_POST['selected_user'])) { $email=$mysqli->real_escape_string($_POST['selected_user']); // Sanifica
    l'input
