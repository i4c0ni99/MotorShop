...
// Verifica se l'ID del prodotto è stato passato via GET
if (isset($_GET['id'])) {
    $productId = $_GET['id'];
    ...
    // Gestisci il caso in cui l'ID del prodotto non è valido o mancante
    $_SESSION['error'] = "ID del prodotto non valido o mancante.";
    header("Location: /MotorShop/dashBoard.php");
    exit();
}

// Salvataggio dei dati del nuovo sottoprodotto
if (isset($_POST['save'])) {
    ...
    if (empty($errors)) {
        ...
        // Esecuzione della query di inserimento del sottoprodotto
        if ($mysqli->query($insertSubProductQuery)) {
            $subProductId = $mysqli->insert_id; // Ottieni l'ID del sottoprodotto appena inserito
